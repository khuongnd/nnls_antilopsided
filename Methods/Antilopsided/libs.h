...
//a product of matrix and vector
// void dotMV(double **a, double *b, int m, int n, double *res){
//    double x, *end=a[0]+(m*n);
//    For(i,m) res[i] = 0;
//    For(j,n) if ((x = b[j]) != 0)
//        for(double *pt=a[0]+j,*r=res; pt<end; pt+=n,r++)
//            (*r) += x*(*pt);
// }
...
int FCDDf(double **H, double *h, int k, double* df, double* x){
    double dx, maxValue = 0, dfx;
    int pos = -1;
    For(iter, k){
        pos = -1;
...
void get1DArray(const mxArray* a, double **res, int *n){
    *res = mxGetPr(a);
    *n = mxGetM(a);
}
