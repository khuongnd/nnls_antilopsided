...
% For details on license and terms please see http://www.gnu.org/copyleft/gpl.html
    ...
    while toc(startTime) < opt.maxTime
        out.iter = out.iter + 1;
        ...
        BB = (out.x > 0 | out.grad < 0);
        his.obj = [his.obj out.obj];
        his.d_barf = [his.d_barf sum(out.grad(BB).^2)];
    ...
    his.out = out;
    his.obj = [his.obj sum((A*his.x - b).^2/2.0)];
    his.finalObj = norm(A*his.x - b, 'fro')^2/2;
    his.d_barf = [his.d_barf double(out.time)];
    his.time = [his.time toc(startTime)];
end
